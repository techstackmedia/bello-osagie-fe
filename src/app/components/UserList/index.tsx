...
const UserList: React.FC<UserListProps> = ({ users, onEditUser, handleShowDelete, handleNewUserButtonClick }: any) => {
    ...
    const handleUpdateUser = async (updatedUser: User) => {
        try {
            const response = await axios.put(`https://caf58c5c7aba97e698b6.free.beeceptor.com/api/users/${updatedUser.id}`, updatedUser);

            if (response.status === 200) {
                const getResponse = await axios.get(`https://caf58c5c7aba97e698b6.free.beeceptor.com/api/users/${updatedUser.id}`);
